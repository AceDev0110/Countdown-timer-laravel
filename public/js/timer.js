document.addEventListener("DOMContentLoaded", () => {
    ...
    const predefinedTimerSelect = document.getElementById(
        "predefinedTimerSelect"
    );
    ...
    function updateTimerColor() {
        ...
        if (window.APP_SIGNIN) {
            if (isGuest) {
                timerElement.classList.add("text-gray-600");
            } else if (remainingTime <= redAlertTime) {
    ...
    saveSettingsButton.addEventListener("click", () => {
        ...
        if (mode === "predefined") {
            ...
            time =
                parseInt(customDurationInput.value, 10) > 0
                    ? parseInt(customDurationInput.value, 10)
                    : 0;
        }
        if (window.APP_SIGNIN == 1) {
            orangeAlertTime =
                parseInt(orangeAlertInput.value, 10) > 0
                    ? parseInt(orangeAlertInput.value, 10)
                    : 0;
            redAlertTime =
                parseInt(redAlertInput.value, 10) > 0
                    ? parseInt(redAlertInput.value, 10)
                    : 0;
        } else {
            orangeAlertTime = redAlertTime = 0;
        }
